...
func clearStats():
	setStats({"currentRank":0, "highestRank":0, "games":0, "wins":0, "dots":0, "deaths":0, "kills":0, "types":{"0":0, "1":0, "2":0}})
...
func setStats(stats:Dictionary):
	...
	$"%Winrate".text = String(floor(float(stats.wins)/max(stats.games, 1)*100))
	$"%AverageDots".text = String(floor(float(stats.dots)/max(stats.games, 1)))
	...
	$"%TypeDistro".value = float(stats.types[highest])/max(stats.games, 1)
